...
public:
	...
	struct Item_t
	{
		...
		{
			int m_Count = GetSelectableCount();
			return (std::to_string(min(m_Count, m_Selected + 1)) + " of " + std::to_string(m_Count));
		}
	...
	struct Colors_t
	{
		ImU32 Header = IM_COL32(17, 85, 166, 255); // the stupid color on gta 5 interaction menu
		ImU32 Title = IM_COL32(0, 0, 0, 255);
		ImU32 Footer = IM_COL32(0, 0, 0, 220);
		ImU32 Item = IM_COL32(0, 0, 0, 180);
		ImU32 ItemSelected = IM_COL32(0, 62, 137, 180);
		ImU32 Separator = IM_COL32(0, 102, 204, 255);
		ImU32 Primary = IM_COL32(0, 102, 204, 255);
	...
	struct Input_t
	{
		...
		__inline void SelectUp() {
			m_SelectUpDown -= 1;
			PlayWavFile(L"gta5_scrolling_menu.wav");
		}
		__inline void SelectDown() {
			m_SelectUpDown += 1;
			PlayWavFile(L"gta5_scrolling_menu.wav");
		}

		char m_SelectLeftRight = 0; // -1 = Left | 1 = Right
		__inline void SelectLeft() {
			m_SelectLeftRight -= 1;
			PlayWavFile(L"gta5_scrolling_menu.wav");
		}
		__inline void SelectRight() {
			m_SelectLeftRight += 1;
			PlayWavFile(L"gta5_scrolling_menu.wav");
		}
		...
		__inline void SelectInteraction() {
			PlayWavFile(L"gta5_button_click.wav");
			m_SelectInteraction = true;
		}
	...
	void End()
	{
		// Header
		{
			...
			if (Header.m_Image) {
				m_DrawList->AddImage((ImTextureID)Header.m_Image, m_DrawPos, m_DrawPos + Header.m_ImageSize);
			}
